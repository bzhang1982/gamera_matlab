...
prob = 'BW2D'; 
T_stop = 0.18; % maximum sim time

global nx ny nz nx_total ny_total nz_total

% generate a grid with (nx,ny,nz) points. For 2-D simulations nz = 1
nx = 256; % # of active cell centers in the first dimension (i-direction)
ny = 256; % # of active cell centers in the second dimension (j-direction)
...
for n=1:N_max
    ...
    if(mod(n,20)==0)
        ...
        subplot(1,2,2)
        pcolor(xc(ic_act,jc_act,kc_act(1)),yc(ic_act,jc_act,kc_act(1)),squeeze(p(ic_act,jc_act,kc_act(1))));
        shading flat;xlabel('x'),ylabel('y');
        title('P')
